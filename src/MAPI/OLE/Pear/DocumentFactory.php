...
{
  private $_temporaryStream = null;

  public function __destruct() {
    // close the stream at end
    if (is_resource($this->_temporaryStream)) {
      // but it seems, that closing in destructor can be premature...
      // fclose($this->_temporaryStream);
    }
  }
  ...
  /**
   * Create the document from string data.
   *
   * @param string $string
   * @return CompoundDocumentElement
   */
  public function createFromString(string $string) : CompoundDocumentElement {
    $this->_temporaryStream = fopen('php://memory', 'r+');
    fwrite($this->_temporaryStream, $string);
    rewind($this->_temporaryStream);
    return $this->createFromStream($this->_temporaryStream);
  }
